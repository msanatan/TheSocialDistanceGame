  ...
  switch (gameState) {
    ...
    case PLAYING: {
      ...
      if (player.justHit) {
        ...
        if (static_cast<int>(lastHitSeconds) >=
            player.getInvincibilityDuration()) {
          player.justHit = false;
        }
      }
      ...
      if (arduboy.justPressed(UP_BUTTON) && player.y > 16) {
        player.y = player.y - player.speedY;
                 ...
                 player.y + player.getSize() < HEIGHT) {
        player.y = player.y + player.speedY;
      }
      ...
      for (int i = 0; i < MAX_PEDESTRIANS; i++) {
        ...
        if (arduboy.collide(pedestrians[i].getRect(), playerRect) &&
            !player.justHit) {
          --player.lives;
          player.justHit = true;
      ...
      // Draw pedestrians
      for (int i = 0; i < MAX_PEDESTRIANS; i++) {
        Sprites::drawOverwrite(pedestrians[i].x, pedestrians[i].y,
                               pedestrianSprite, pedestrians[i].frame);
      }
      ...
      if (!player.justHit) {
        ...
        if (arduboy.everyXFrames(6)) {
          Sprites::drawOverwrite(player.x, player.y, playerSprite,
                                 player.frame);
        }
      }
    } break;
